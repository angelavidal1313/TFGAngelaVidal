...
delta_beta <- function(x, par, t, r){
  ...
  # INPUT
  # x= [x(1) x(2) x(3) x(4)] the simbolic parameters that characterizes the MAP in canonical form.
  # par= [par(1) par(2) par(3) par(4) par(5) par(6)]. Where par(1) and par(2)
  # are the weight given in the optimization problem, par(3) are the first
  # lag autocorrelation function, par(4), par(5) and par(6) are the first,
  # the second and the third interarrival moments.
  # Vector t of interval lenghts for counting arrivals.
  #  r is the vector of correlation counting process.
...
calcular_descriptores_empiricos <- function(data, top_percent = 0.01) {
  ...
  # --- Cálculo de los momentos empíricos ---
  ...
  # --- Cálculo de rho (autocorrelación) ---
...
n_tau_with_correlations <- function(datos, tau, hours) {
  ...
  # Retornar tanto la matriz como las estadísticas
  return(list(
